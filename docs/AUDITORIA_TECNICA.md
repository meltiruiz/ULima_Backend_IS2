...
### 6.1 🔴 Errores tragados en silencio (el problema de fondo)
- **Backend: 89 `console.error`** con patrón "atrapo el error de BD y devuelvo fallback silencioso" (200 con `[]`/`null`/datos falsos). El peor: **`academic-profile.repository.ts` devuelve un perfil FICTICIO** (`fullName:"Usuario"`, `code:"00000000"`, career `"--"`) con status 200 ante error de BD, en vez de propagar. Mismo anti-patrón en `course-detail.routes.ts` (4 catches), `schedule.repository.ts` (16), `curriculum`, `grades`, `attendance-risk`, etc.
- **Frontend: `try/catch` que devuelven `[]`** (p.ej. `asesoria_service.fetchAsesorias`) → una falla real se ve como "no hay datos".
- **Efecto combinado:** cuando algo se rompe (un endpoint movido, una query fallida), **la app no muestra error: muestra vacío o datos inventados.** Por eso los bugs de la semana salían como pantallas en blanco sin pista. → **Propagar errores reales (o loguear con señal visible) en vez de tragarlos.**
...
1. **Dejar de tragar errores.** Backend: propagar 500 reales (o al menos no devolver perfiles/listas falsas). Frontend: que un `catch` no se disfrace de "sin datos" — mostrar un estado de error real.
